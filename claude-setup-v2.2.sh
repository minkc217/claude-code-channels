#!/bin/bash
# Claude Code 자동 설정 스크립트 v2.0
# 사용법: bash claude-setup-v2.0.sh
...
echo "========================================"
echo "  Claude Code 자동 설정 스크립트 v2.2"
echo "========================================"
echo ""
...
check_restart_loop() {
  if [ -f "$RESTART_COUNT_FILE" ]; then
    ...
    if [ "$ELAPSED" -lt 600 ]; then
      COUNT=$((COUNT + 1))
      if [ "$COUNT" -ge 3 ]; then
        log "ACTION: 재시작 루프 감지 (10분 내 ${COUNT}회) → 재시작 중단"
        send_telegram "🛑 재시작 루프 감지! 10분 내 ${COUNT}회 재시작 시도. 자동 재시작을 중단합니다. 수동 확인 필요."
...
- 독립에이전트 tmux 소켓: /tmp/tmux-claude-ind
...
- 10분 내 3회 이상 재시작 시도 → 자동 재시작 중단 + 텔레그램 "수동 확인 필요" 알림만
...
- 동작:
  1. 기존 telegram 플러그인 프로세스 kill
  2. 커스텀 소켓($TMUX_SOCKET)으로 tmux 새 세션 생성
  3. 세션 안에서 claude 실행 (--channels plugin:telegram, --dangerously-skip-permissions)
  4. tmux ls로 확인
...
- 동작:
  ...
  6. 독립에이전트 하트비트 순회
- 재시작 루프 방지: 10분 내 3회 이상 재시작 시 중단 + 텔레그램 알림만
...
# ========================================
echo ""
echo "[11/13] crontab 등록..."
CRON_LINE="* * * * * /bin/bash $WATCHDOG_SCRIPT >> $WATCHDOG_LOG 2>&1"

# 기존 crontab 유지 + watchdog 추가 (중복 방지)
# 임시 파일로 처리하여 stdin 충돌 방지
CRON_TMP=$(mktemp)
...
if [ -f "$GLOBAL_CLAUDE/settings.json" ]; then
  ...
  grep -q "telegram@claude-plugins-official" "$GLOBAL_CLAUDE/settings.json" && ok "텔레그램 플러그인 활성화됨" || fail "텔레그램 플러그인 미활성화"
...
if [ "$FAIL" -eq 0 ]; then
  ...
  if [ "$DO_RESTORE" = "y" ]; then
    ...
    echo "  재설치: bash claude-setup-v2.0.sh"
    echo ""
  fi
else
  echo "  $FAIL개 항목에 문제가 있습니다."
...
if echo "$RESPONSE" | grep -q '"ok":true'; then
  ...
  # 테스트 메시지 전송
  curl -s -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" \
    -d chat_id="$CHAT_ID" -d text="🎉 Claude Code 설정 완료! ($AGENT_NAME) v2.0" > /dev/null 2>&1
  ok "테스트 메시지 전송 완료"
else

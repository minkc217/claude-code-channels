#!/bin/bash
# Claude Code 자동 설정 스크립트 v2.6
# 사용법: bash claude-setup-v2.6.sh
...
echo "========================================"
echo "  Claude Code 자동 설정 스크립트 v2.6"
echo "========================================"
echo ""
...
if [ "$FAIL" -eq 0 ]; then
  ...
  if [ "$DO_RESTORE" = "y" ]; then
    ...
    echo "  재설치: bash claude-setup-v2.6.sh"
    echo ""
  fi
else
  echo "  $FAIL개 항목에 문제가 있습니다."
...
# 테스트 메시지 전송
curl -s -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" \
  -d chat_id="$CHAT_ID" -d text="🎉 Claude Code 설정 완료! ($AGENT_NAME) v2.6" > /dev/null 2>&1
ok "테스트 메시지 전송 완료"
